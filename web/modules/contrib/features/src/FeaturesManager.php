...
class FeaturesManager implements FeaturesManagerInterface {
  ...
  public function assignConfigPackage($package_name, array $item_names, $force = FALSE) {
    ...
    foreach ($item_names as $item_name) {
      if (!isset($config_collection[$item_name])) {
        throw new \Exception($this->t('Failed to assign @item_name to package @package_name. Configuration item not found.', [
          '@item_name' => $item_name,
  ...
  public function listExtensionConfig($extension) {
    ...
    $module_list = array();
      if (isset($module_list_scan[$name])) {
        $module_list[$name] = $module_list_scan[$name];
      }
    return array_keys($this->extensionStorage->getComponentNames($module_list));
  }
